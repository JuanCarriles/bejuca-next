    ...
    <div style={{ minHeight: 'calc(100vh - 120px)' }} className="w-full flex flex-col items-center justify-center p-6">
      <div className={`max-w-lg w-full text-center p-10 md:p-14 rounded-3xl shadow-xl transition-all ${isDark ? "bg-[#1a2a3a] border border-gray-800 text-white" : "bg-white border border-gray-100 text-gray-900"}`}>
        <div className="flex justify-center mb-8">
          <div className="rounded-full bg-yellow-500/10 p-5 inline-flex items-center justify-center ring-[12px] ring-yellow-500/5">
            <Clock className="w-16 h-16 text-yellow-500" />
          </div>
        </div>
        
        <h1 className="text-4xl md:text-5xl font-extrabold mb-6 tracking-tight">
          {isEn ? "Payment Pending" : "Pago en Revisión"}
        </h1>
        
        <p className={`text-lg mb-10 leading-relaxed ${isDark ? "text-gray-400" : "text-gray-600"}`}>
          {isEn 
            ? "Your payment is currently being processed. Once it clears, you will receive a confirmation email with course access details."
            : "Tu pago se está procesando. Una vez que se acredite, recibirás un correo de confirmación con los accesos al curso."}
        </p>
